...
def generate_launch_description():
    ...
    bringup_dir = get_package_share_directory('pb_swc_simulation')
    pkg_gazebo_ros = get_package_share_directory('gazebo_ros')

    # Specify xacro path
    default_robot_description = Command(['xacro ', os.path.join(
    get_package_share_directory('pb_swc_simulation'), 'urdf', 'simulation_waking_robot.xacro')])
    ...
    append_enviroment = AppendEnvironmentVariable(
        'GAZEBO_PLUGIN_PATH',
        os.path.join(os.path.join(get_package_share_directory('pb_swc_simulation'), 'meshes', 'obstacles', 'obstacle_plugin', 'lib'))
    )
