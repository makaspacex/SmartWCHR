...
pthread_t tid1,tid2;
char cmd[] = "python3 serial_d.py";
char cmd_buf [100] = {'\0'};

const char * ASR_RES_PATH        = "fo|res/asr/common.jet"; //离线语法识别资源路径
const char * GRM_BUILD_PATH      = "res/asr/GrmBuilld"; //构建离线语法识别网络生成数据保存路径
const char * GRM_FILE            =	"/home/jetson/Desktop/SmartWCHR/src/swc_perception/voice_ctrl/resource/Offline_code/bin/call.bnf";
...
const char* get_audio_file(void)
{
	...
	while(key != 27) //按Esc则退出
	{
		...
		case 1:
			printf("\n1.打电话给丁伟\n");
			return "wav/text.wav"; //ddhgdw.pcm  
		case 2:
			printf("\n2.打电话给黄辣椒\n");
...
int update_lexicon(UserData *udata)  //更新词的
{
...
void writeStringToFile(const char* filename, const char* content) {
    // 打开文件，使用 "w" 模式表示写入（如果文件不存在将创建文件，如果文件存在将覆盖内容）
	...
	printf("write result to txt file\n\n");
...
/* demo recognize the audio from microphone */ //这函数是从mic直接录入，但打不开MIC设备，所以弃用
...
int run_asr(UserData *udata)
{
	...
	{
		//demo_mic(asr_params);
		printf("正在检测说话,5秒后自动停止...\n");
		system("arecord -D hw:2,0 -r 16000 -f s16_LE -d 5 -t wav ./wav/irecode.wav");
		sleep(5);
		printf("说话完毕，识别中...\n");
		asr_audiof = "wav/irecode.wav";
...
void * record_shibei(void * S)//线程2
{
	const char *login_config    = "appid = 66c52f76"; //登录参数                 e42666b8
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
...
int main(int argc, char* argv[])  
{
  ...
  err1 = pthread_create(&tid1, NULL,strat_main, NULL);
  err2 = pthread_create(&tid2, NULL,record_shibei, NULL);
  if(err1 < 0)
  {
  ...
  if(err2 < 0)
  {
  	perror("pthread2_create fail \n");
  	return -1;
  }
	
	do
	{
		printf("please input ""exit"" to stop!\n");
		scanf("%s",exit);
	}while(strncmp(exit,"exit",4));
	
	pthread_cancel(tid1);
	strcat(cmd_buf,"killall ");
	strcat(cmd_buf,cmd);
	system(cmd_buf);
	pthread_cancel(tid2);
