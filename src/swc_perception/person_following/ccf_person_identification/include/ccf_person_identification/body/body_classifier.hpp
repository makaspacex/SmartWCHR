...
public:
    ...
    BodyClassifier(rclcpp::Node::SharedPtr node)
        : num_positives(0),
          num_negatives(0)
    {
	...
	// std::string package_path = ament_index_cpp::get_package_prefix("ccf_person_identification");
        ...
        if(channel_types.find("cnn25") != std::string::npos) {
          RCLCPP_INFO(node->get_logger(), "add cnn25 to channel bank");
        ...
        int num_selectors = node->declare_parameter<int>("num_selectors", 15);
        int num_weak_classifiers = node->declare_parameter<int>("num_weak_classifiers", 15);
