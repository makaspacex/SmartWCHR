...
public:
    PersonClassifier(rclcpp::Node::SharedPtr node)
    {
        if(node->declare_parameter<bool>("use_body", true)) {
            classifiers.push_back(std::make_shared<BodyClassifier>(node));
        }
    }
    ...
    virtual boost::optional<double> predict(const Features::Ptr& features) override {
       //  RCLCPP_ERROR(get_logger(), "This method must not be called!!");
        abort();
        return boost::none;
    }
